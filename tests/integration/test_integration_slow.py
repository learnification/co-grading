...
def verify_score_submission(client, task_id):
    ...
    score_response = client.post("/api/v1/scores", json={"evaluation_id": task_id, "score": 1})
...
def test_real_multiple_submissions(multiple_submissions_request):
    """Test that a grading request (with multiple submissions) completes a Celery task successfully with real LLM."""
    with TestClient(app=app) as client:
        # Create the grading task
