...
@pytest.mark.parametrize("content_type", ["image/jpeg", "text/plain", "application/json", "application/msword", "video/mp4"])
def test_various_invalid_content_types(content_type):
    """Test that various non-PDF content types are rejected."""
    response = client.post("/api/v1/documents/upload", files={"file": ("test_file", b"fake content", content_type)})
...
def test_upload_success(mocker):
    ...
    response = client.post("/api/v1/documents/upload", files={"file": ("test.pdf", pdf_content, "application/pdf")})
